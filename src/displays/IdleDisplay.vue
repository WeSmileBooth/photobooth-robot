...
async function generateQRCode() {
  ...
  }?session=${sessionId}&ngrok=${ngrokUrl}`;
...
<template>
  <div
    class="absolute inset-0 bg-gradient-to-b from-blue-50 to-white flex flex-col"
  >
    <!-- Header Section -->
    ...
    <main
      class="flex-1 flex flex-col items-center justify-center gap-16 md:gap-20 px-8"
    >
      <!-- QR Code Section -->
      <div
        v-if="isQRCodeLoading"
        class="animate-pulse w-64 md:w-96 h-64 md:h-96 bg-gray-300 rounded-lg"
      ></div>
      <div
        v-else-if="error"
        class="text-red-500 text-2xl md:text-3xl text-center"
      >
        {{ error }}
      </div>
      <div
        v-else
        class="bg-white p-10 md:p-12 rounded-2xl shadow-lg hover:shadow-xl transition-shadow"
      >
        <div
          class="w-64 md:w-96 h-64 md:h-96 flex items-center justify-center"
          v-html="qrCodeSvg"
        ></div>
        <p
          class="text-center mt-8 text-gray-700 text-2xl md:text-3xl font-medium"
        >
